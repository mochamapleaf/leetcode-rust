...
impl Solution {
        ...
        if n as usize > my_vec.len() / 2 {
            ...
            unsafe {
                std::ptr::copy(
                    ...
                    my_vec
                        .as_mut_ptr()
                        .offset(original_len as isize - n as isize),
                    n as usize,
                );
                std::ptr::copy(
                    my_vec.as_ptr().offset(original_len as isize),
                    my_vec.as_mut_ptr(),
                    original_len - n as usize,
                );
            ...
            unsafe {
                std::ptr::copy(
                    my_vec.as_ptr().offset(n as isize),
                    my_vec.as_mut_ptr(),
                    original_len - n as usize,
                );
                std::ptr::copy(
    ...
    pub fn reverse_left_words_unsafe_opt(mut s: String, n: i32) -> String {
        ...
        if n as usize > my_vec.len() / 2 {
            ...
            unsafe {
                std::ptr::copy(
                    my_vec.as_ptr(),
                    my_vec.as_mut_ptr().offset(temp_vec.len() as isize),
                    n as usize,
                );
            ...
            unsafe {
                std::ptr::copy(
                    my_vec.as_ptr().offset(n as isize),
                    my_vec.as_mut_ptr(),
                    my_vec.len() - n as usize,
                );
                std::ptr::copy(
                    temp_vec.as_ptr(),
                    my_vec
                        .as_mut_ptr()
                        .offset(my_vec.len() as isize - n as isize),
                    n as usize,
                );
                String::from_utf8_unchecked(my_vec)
            }
    ...
    pub fn reverse_left_words_unsafe_bad(mut s: String, n: i32) -> String {
        ...
        unsafe {
            std::ptr::copy(
                ...
                my_vec.as_mut_ptr(),
                original_len,
            ); //third copy here, copied len items
    ...
    pub fn reverse_left_words_no_buffer(mut s: String, n: i32) -> String {
        ...
        my_vec[n as usize..].reverse();
        my_vec.reverse();
        unsafe { String::from_utf8_unchecked(my_vec) }
    }
}
