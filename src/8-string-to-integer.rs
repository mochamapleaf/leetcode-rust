...
impl Solution {
  pub fn my_atoi(s: String) -> i32 {
    ...
    while let Some(x) = s_iter.next(){
      if x == b' '{ continue; } //skip if whitespace
      else if x == b'-'{
        is_neg = true;
        break
      }else if x == b'+'{ break; } //remember the symbol, and exit
      else if x.is_ascii_digit(){ //no sign meaning the sign is '+' and omitted
        temp_num = (x - b'0') as i32; //store the number
        break;
      }else{ return 0; }
    }
    if is_neg{ temp_num = -temp_num; } //if the number is negative, we need to use '-' every time
...
fn test_solution(){
  ...
  assert_eq!(Solution::my_atoi("-2147483647".to_string()), -2147483647_i32);
